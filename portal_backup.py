...
def mostrar_portal(filtro_tipo="MASTER", filtro_valor=None):
    ...
    if not base.empty:
        ...
        with tab2:
            ...
            opcoes_motivo = ["Todos os motivos"] + motivos_disponiveis

            if "filtro_motivo_pedidos" not in st.session_state:
                st.session_state["filtro_motivo_pedidos"] = "Todos os motivos"
            if st.session_state["filtro_motivo_pedidos"] not in opcoes_motivo:
                st.session_state["filtro_motivo_pedidos"] = "Todos os motivos"

            col_mot_label, col_mot_sel, col_mot_esp = st.columns([1.2, 3, 5])
            with col_mot_label:
                st.markdown(
                    "<div style='padding-top:8px;font-size:13px;font-weight:600;"
                    "color:#374151;'>🔖 Motivo:</div>",
                    unsafe_allow_html=True
                )
            with col_mot_sel:
                motivo_sel = st.selectbox(
                    ...
                    index=opcoes_motivo.index(st.session_state["filtro_motivo_pedidos"]),
                    key="sel_motivo_pedidos",
            ...
            def aplicar_filtro_motivo(df):
                if motivo_ativo == "Todos os motivos":
                    return df
                return df[df["Motivo"].astype(str) == motivo_ativo]
            ...
            with ft1:
                renderizar_tabela(aplicar_filtro_motivo(pedidos_view))
            with ft2:
                renderizar_tabela(aplicar_filtro_motivo(pedidos_view[pedidos_view["Status"].str.lower() == "pendente"]))
            with ft3:
                renderizar_tabela(aplicar_filtro_motivo(pedidos_view[pedidos_view["Status"].str.lower() == "liberado"]))
            with ft4:
                renderizar_tabela(aplicar_filtro_motivo(pedidos_view[pedidos_view["Status"].str.lower() == "conferido"]))
            with ft5:
                renderizar_tabela(aplicar_filtro_motivo(pedidos_view[pedidos_view["Status"].str.lower() == "batch"]))
